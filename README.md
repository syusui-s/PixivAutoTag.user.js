...
* コメント行の規則
	...
	* ルール行でコメントのために `#` を用いることはできない。例えば、`pattern A A' A'' # コメント` とすることはできない。
* ルール文の規則
	* ルール文の区切りは空白文字である。
...
注意事項
-----
[ローカルストレージ(localStorage)](https://developer.mozilla.org/ja/docs/Web/API/Window/localStorage)にタグ付けルールを保存しているため、閲覧履歴の削除機能等を使用するとルールが消失してしまいます。
誤ってローカルストレージを消してしまった場合に備えて、閲覧履歴を消す前に「設定のダウンロード」を使ってバックアップを取ることを推奨します。
データの消失には対応いたしかねますので、ご注意ください。
